...
public class SortingAlgorithmFactory extends GeneticFactory<ProgramChromosome, Instruction> {

	private Memory _in, _wk;
	private int[][] _tests;
	private int _length;
	
	public SortingAlgorithmFactory(int length) {
		_in = new Memory("in", 30, 0);
		_wk = new Memory("wk", 5, 0);
		_length = length;
		
		_tests = new int[50][];
		for(int i = 0; i < _tests.length; i++) {
			_tests[i] = new int[(int) (Math.random() * 30)];
	...
	public Instruction getRandomGene() {
		switch((int) (Math.random() * 5)) {
			...
			default:
				ImmediateOperand line = new ImmediateOperand(-_length, _length);
				Operand o1 = (Math.random() < 0.5) ? new DirectOperand(_wk) : new IndirectOperand(_wk, _in);
				Operand o2 = (Math.random() < 0.5) ? new DirectOperand(_wk) : new IndirectOperand(_wk, _in);
