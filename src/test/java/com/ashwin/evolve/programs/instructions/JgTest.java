...
public class JgTest {
	...
	@Test
	public void testExecuteGreater() throws ExecutionException {
		when(_lines.read()).thenReturn(0);
		when(_src.read()).thenReturn(1);
		when(_dest.read()).thenReturn(0);
		
		Jg instr = new Jg(_lines, _src, _dest);
	...
	@Test
	public void testExecuteLessThanEqualTo() throws ExecutionException {
		when(_lines.read()).thenReturn(0);
